...
public class KernelTestModuleController extends BaseModuleController {
    ...
    @Option(
            name = "is-low-mem",
            description =
                    "If this option set to true, run this module if device prop"
    ...
    public RunStrategy shouldRun(IInvocationContext context) {
        ...
        if (mIsLowMem) {
            ...
            if (deviceLowMem(context)) {
                CLog.d(
                        "Skipping module %s because the test is not for low memory device.",
                        getModuleName());
                return RunStrategy.FULL_MODULE_BYPASS;
            }
        }
        ...
        if (mIsHwasan) {
            if (!deviceWithHwasan(context)) {
                CLog.d(
                        "Skipping module %s because %s is not ended with _hwasan.",
                        getModuleName(), productNameProp);
                return RunStrategy.FULL_MODULE_BYPASS;
            }
        } else {
            if (deviceWithHwasan(context)) {
                CLog.d(
                        "Skipping module %s because the test is for device of hwasan.",
                        getModuleName());
                return RunStrategy.FULL_MODULE_BYPASS;
            }
        }
    ...
    private boolean deviceWithHwasan(IInvocationContext context) {
        for (ITestDevice device : context.getDevices()) {
            ...
            try {
                ...
                CLog.e(
                        "Couldn't check prop of %s on %s",
                        productNameProp, device.getSerialNumber());
                CLog.e(e);
                throw new RuntimeException(e);
            }
        }
